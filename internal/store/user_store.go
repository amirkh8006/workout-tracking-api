...
func (p *password) Set(plainTextString string) error {
	hash, err := bcrypt.GenerateFromPassword([]byte(plainTextString) , 12)
	if err != nil {
		return err
	}

	p.plainText = &plainTextString
	p.hash = hash
	return nil
}

func (p *password) Matched(plainTextString string) (bool, error ) {
